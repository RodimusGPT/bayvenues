...
// Generate venue INSERT statements
...
for (let batch = 0; batch * BATCH_SIZE < venues.length; batch++) {
  ...
  fs.writeFileSync(path.join(__dirname, `venue-batch-${batch}.sql`), sql);
  console.log(`Generated batch ${batch}: venues ${start}-${end-1}`);
}

console.log(`\nTotal batches: ${Math.ceil(venues.length / BATCH_SIZE)}`);
