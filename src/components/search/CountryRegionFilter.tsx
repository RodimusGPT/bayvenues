...
import { useRegionMetadata, type CountryData } from '../../hooks/useRegionMetadata';
...
function CountryRow({
  ...
  const selectedRegionCount = country.regions.filter(r =>
  ...
  const filteredCountries = useMemo(() => {
    ...
    return countries
      .map(country => {
        ...
        // Filter regions that match
        const matchingRegions = country.regions.filter(r =>
          r.name.toLowerCase().includes(query)
        );

        // Include country if name matches OR has matching regions
        if (countryMatches || matchingRegions.length > 0) {
          return {
            ...country,
            // If searching by region, show only matching regions; otherwise show all
